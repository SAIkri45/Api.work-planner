import { minLength, nonEmpty, object, optional, picklist, pipe, string, transform } from "valibot";
...
export const VCreateTaskSchema = object({
    ...
    project_id: pipe(string(TASK_PROJECT_ID_INVALID), nonEmpty(TASK_PROJECT_ID_MISSING), transform(value => Number(value))),
    task_status: pipe(string(TASK_STATUS_INVALID), transform(value => value.trim()), picklist(allowedTaskStatuses, TASK_STATUS_INVALID)),
    start_date: pipe(string("Invalid start date"), transform(value => value.trim())),
    end_date: pipe(string("Invalid end date"), transform(value => value.trim())),
});
