...
export const userPassword = pipe(
  string(PASSWORD_REQUIRED),
  nonEmpty(PASSWORD_REQUIRED),
  minLength(8, PASSWORD_MIN_LENGTH),
);

export const userPhone = pipe(
  string(USER_PHONE_REQUIRED),
  nonEmpty(USER_PHONE_REQUIRED),
  regex(/^\d+$/, INVALID_PHONE_NUMBER),
  regex(/^[6-9]\d{9}$/, INVALID_PHONE_NUMBER),
);
