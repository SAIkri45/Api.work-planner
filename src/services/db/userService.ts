...
export async function getAllRemovedProject(
  userId: number,
  offset?: number,
  pageSize?: number,
  search?: string,
  ...
  const removedProjects = result.filter((project: any) => {
    const deletedRows = project.userProjects.filter((userProject: any) => userProject.deleted_at !== null);
    const activeRows = project.userProjects.filter((userProject: any) => userProject.deleted_at === null);
    return deletedRows.length > 0 && activeRows.length === 0;
  });

  const mappedResult = removedProjects.map((project: any) => ({
    id: project.id,
    title: project.title,
