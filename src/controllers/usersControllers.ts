...
export class UsersController {
  ...
  updateUser = async (c: Context) => {
    ...
    const userData = await getSingleRecordByMultipleColumnValues<User>(users, ["id", "deleted_at"], [userId, null]);
    if (!userData) {
      throw new NotFoundException(USER_NOT_FOUND);
    }
    const { password, ...result } = await updateRecordById<User>(users, userId, validatedReq);
